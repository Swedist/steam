...
from lib.steam.utils.subs import load_json
...
class CsmoneyParser(BaseParser):
    ...
    def _csmoney_skin_parser(cls, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        item_price = {
            ...
            "extra_price": (
                sum(list(item.get("pricing", {}).get("extra", {}).values()))
                if bool(item.get("pricing", {}).get("extra", {}))
                else 0
            ),
    ...
    def _run(self) -> Dict[str, Any]:
        ...
        while True:
            for item in range(offset, offset + batch_size, 60):
                ...
                items = self._create_request(
                    url=temp_url, headers=self.headers, proxies=self.proxies
                )

                temp_result = (
                    list(map(lambda elem: self._csmoney_skin_parser(item=elem), items))
                    if items
                    else []
                )
    ...
    def _save(self) -> None:
        ...
        with open(
            os.path.join(self.PARSER_OUTPUT_PATH, file_name), mode="w", encoding="utf-8"
        ) as file:
...
class SteamParser(BaseParser):
    ...
    def _steam_skin_parser(cls, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # get item price
        item_price = {"computed_price": item[0]}

        return {"price": item_price}

    def _run(self) -> Dict[str, Any]:
        url_params = {"steam_item_id": steam_db[self.name]["steam_id"]}
        url = self._create_href(**url_params)
        items = self._create_request(url=url, headers=self.headers, proxies=self.proxies)
        self.result = (
            list(map(lambda elem: self._steam_skin_parser(item=elem), items)) if items else []
        )
